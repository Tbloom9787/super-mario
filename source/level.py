...
import json
import pygame
from source import game_functions, player, coin, enemy, scoreboard, box, objects, brick, settings
from source import constants as c


class Level(game_functions.State):
    def __init__(self):
    ...
    def startup(self, current_time, persist):
        ...
        self.overhead_info = scoreboard.Scoreboard(self.game_info, c.LEVEL)
        self.load_map()
    ...
    def load_map(self):
        ...
        file_path = os.path.join('assets', 'data', map_file)
        f = open(file_path)
    ...
    def setup_background(self):
        ...
        self.background = settings.GFX[img_name]
        ...
        self.level = pygame.Surface((self.bg_rect.w, self.bg_rect.h)).convert()
        self.viewport = settings.SCREEN.get_rect(bottom=self.bg_rect.bottom)
    ...
    def setup_collide(self, name):
        ...
        if name in self.map_data:
            for data in self.map_data[name]:
                group.add(objects.Collision(data['x'], data['y'],
    ...
    def setup_pipe(self):
        ...
        if c.MAP_PIPE in self.map_data:
            for data in self.map_data[c.MAP_PIPE]:
                self.pipe_group.add(objects.Pipe(data['x'], data['y'],
        ...
        if c.MAP_SLIDER in self.map_data:
            for data in self.map_data[c.MAP_SLIDER]:
                if c.VELOCITY in data:
                    ...
                    vel = 1
                self.slider_group.add(objects.Slider(data['x'], data['y'], data['num'],
    ...
    def setup_brick_and_box(self):
        ...
        self.brickpiece_group = pygame.sprite.Group()
    ...
    def setup_checkpoints(self):
        ...
        for data in self.map_data[c.MAP_CHECKPOINT]:
            ...
            if c.MAP_INDEX in data:
                ...
                map_index = 0
            self.checkpoint_group.add(objects.Checkpoint(data['x'], data['y'], data['width'],
        ...
        if c.MAP_FLAGPOLE in self.map_data:
            for data in self.map_data[c.MAP_FLAGPOLE]:
                if data['type'] == c.FLAGPOLE_TYPE_FLAG:
                    sprite = objects.Flag(data['x'], data['y'])
                    self.flag = sprite
                elif data['type'] == c.FLAGPOLE_TYPE_POLE:
                    sprite = objects.Pole(data['x'], data['y'])
                else:
                    sprite = objects.PoleTop(data['x'], data['y'])
    ...
    def update_all_sprites(self, keys):
        if self.player.dead:
            self.player.update(keys, self.game_info)
    ...
    def check_checkpoints(self):
        ...
        if checkpoint:
            if checkpoint.type == c.CHECKPOINT_TYPE_ENEMY:
                ...
                self.flagpole_group.add(objects.CastleFlag(8745, 322))
    ...
    def check_is_falling(self, sprite):
        sprite.rect.y += 1
        check_group = pygame.sprite.Group(self.ground_step_pipe_group,
    ...
    def update_score(self, score, sprite, coin_num=0):
        ...
        x = sprite.rect.x
        y = sprite.rect.y - 10
        self.moving_score_list.append(scoreboard.GameStats(x, y, score))
